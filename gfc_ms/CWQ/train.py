...
from CWQ_t.data import load_data
from CWQ_t.model import GFC
from CWQ_t.predict import validate
import logging
...
def train(args):
    ...
    ent2id, rel2id, train_loader, val_loader = load_data(input_dir, args.bert_name, args.batch_size, args.rev)
    ...
    optimizer_grouped_parameters = [{'params': bert_param, 'weight_decay': 0.01, 'lr': args.bert_lr},
                                    {'params': other_param, 'weight_decay': 0.01, 'lr': args.lr}]
    ...
    def forward_fn(topic_entity, question, answer,triples, entity_range):
        loss = model.construct(topic_entity, question, answer,triples, entity_range)
        logits = 0
        return loss, logits
    ...
    def train_step(data):
        (loss, _), grads = grad_fn(data[0], data[1], data[2], data[3], data[4])
        optimizer(grads)
        return loss
    ...
    for epoch in range(20):
        ...
        if epoch % 1 == 0:
            p_info = validate(model, val_loader, epoch + 1)
            logging.info(p_info)

    mindspore.save_checkpoint(model, "cwq.ckpt")
...
def main():
    ...
    # training parameters
    parser.add_argument('--bert_lr', default=0, type=float)
