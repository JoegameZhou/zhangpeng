...
def load_data(input_dir, bert_name, batch_size):
    cache_fn = os.path.join(input_dir, 'processed_hop_ms_metaqa.pt')
    if os.path.exists(cache_fn):
        ...
        triples = mindspore.Tensor(triples)
        train_data = Dataset(input_dir, os.path.join(input_dir, 'qa_train_webqsp_fixed_hop.txt'), bert_name, ent2id, rel2id, training=True)
