...
sparse_dense_matmul = mindspore.ops.SparseTensorDenseMatmul(adjoint_dt=True)
...
class GFC(nn.Cell): #
    def __init__(self, args, ent2id, rel2id, triples):
        ...
        print('triple size: {}'.format(self.Tsize))
        try:
        ...
        self.idx2 = mindspore.Tensor([i for i in range(self.Tsize)])
    ...
    def follow(self, e, r):
        ...
        x = x1*x2
        op = ops.Stack()
        indices = op((self.idx2.astype(mindspore.int32), self.triples[:,2].astype(mindspore.int32))).transpose()
        output = sparse_dense_matmul(indices, mindspore.Tensor([1] * self.Tsize, mindspore.float32), (self.Tsize, self.Esize), x)
        # output = ops.csr_mm(self.Mobj, x,trans_a=True)
        return output.transpose((1, 0)).squeeze(0)  # [bsz, Esize]
