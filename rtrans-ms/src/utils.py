...
def LoadNewestCkpt(load_finetune_checkpoint_dir, prefix):
    ...
    max_time = 0
    load_finetune_checkpoint_all_path = []
    for filename in files:
        if filename.startswith(prefix) and filename.endswith(".ckpt"):
            ...
            load_finetune_checkpoint_all_path.append([mtime, full_path])
            if mtime > max_time:
                max_time = mtime
                load_finetune_checkpoint_path = full_path
    load_finetune_checkpoint_all_path = [item[1] for item in sorted(load_finetune_checkpoint_all_path, reverse=True)]
    print("Find the newest checkpoint: ", load_finetune_checkpoint_path)
    return load_finetune_checkpoint_path, load_finetune_checkpoint_all_path # return all ckpt in order reverse by time
...
def long_sequence_splitter(batch, window_len: int, pretrained_model):
    """
    batch:       [bs, max_len]
        example:
        {
         'input_ids': tensor([[1037,  2003,  1042,  2078, 23969,  2546,  1055,  1042, 15992,
              3619,  2546, 16233,  8950,  1042, 22747,  1050,  2546, 22889]]), 
    ...
    res = []
    if pretrained_model=='mT5':
        for i, j in get_seq_indices(seq_len, window_len):
            res.append((batch['input_ids'][:, i:j], batch['attention_mask'][:, i:j], batch['attention_mask'][:, i:j]))
    else:
        for i, j in get_seq_indices(seq_len, window_len):
            res.append((batch['input_ids'][:, i:j], batch['token_type_ids'][:, i:j], batch['attention_mask'][:, i:j]))
    return res
