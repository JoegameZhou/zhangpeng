...
def do_train(dataset=None, network=None, load_checkpoint_path="", save_checkpoint_path="", epoch_num=1):
    ...
    print(f'All {steps_per_epoch} steps in training stage.')
    # optimizer
    ...
    model.train(epoch_num, dataset, callbacks=callbacks, dataset_sink_mode=False) # 数据下沉模式dataset_sink_mode，此参数默认True需设置成False，因为此模式不支持CPU计算平台。
...
def do_eval(dataset=None, network=None, num_class=2, assessment_method="accuracy", load_checkpoint_path=""):
    ...
    if assessment_method == "accuracy":
        ...
        # callback = F1(False, num_class, mode='MultiLabel')
    elif assessment_method == "mcc":
        callback = MCC()
    elif assessment_method == "spearman_correlation":
...
def run_classifier():
    ...
    if args_opt.do_train.lower() == "true":
        ...
        if args_opt.do_eval.lower() == "true":
            ...
            load_finetune_checkpoint_path, load_finetune_checkpoint_all_path = LoadNewestCkpt(load_finetune_checkpoint_dir, "classifier")
